...
STEP: for my $i (0..$#in1-1){
	...
	if ($chri ne $chrin){ next STEP; } #skips end of RACF and one-block RACFs
		...
		if (not defined $idx){ $flag1 = "1stNotFound"; $status = "NotMaintained"; }
		my ($idxn, $warn) = &find_overlap($i+1, "second");
		if (not defined $idxn){ $flag2 = "2ndNotFound"; $status = "NotMaintained"; }
		if ($war eq "zeroOverlap" or $warn eq "zeroOverlap") { $status = "extraAdj"; }
		#print "$idx\t$war\t$idxn\t$warn\t$status\n"; 
		# print not founds and move to next
		if ($status eq "NotMaintained" or $status eq "extraAdj"){
