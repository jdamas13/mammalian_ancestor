...
while (<IN1>) {
	...
	if ($_ =~ /CONSENSUS/ && $_ =~ /$sps/ && $_ !~ /COMP_GEN/) {
		...
		my @tmp = split(/,/,$t);
		#print Dumper @tmp;
		if($tmp[6] eq "-1"){ $tmp[6] = "-1";}
		if($tmp[6] eq "+1" || $tmp[6] eq "1"){ $tmp[6] = "+1";}
		@{$in1[$x]} = ($tmp[1], $tmp[2], $tmp[3], "chr$tmp[8]", $tmp[4], $tmp[5], $tmp[6]);
		# 	APCF start end species start end or
		$x++;
	}	
}
...
while (<IN2>) {
	...
	if ($_ =~ /CONSENSUS/ && $_ =~ /$sps/ && $_ !~ /COMP_GEN/) {
		...
		if($tmp[6] eq "-1"){ $tmp[6] = "-1";}
		if($tmp[6] eq "+1" || $tmp[6] eq "1"){ $tmp[6] = "+1";}
		@{$in2[$y]} = ($tmp[1], $tmp[2], $tmp[3], "chr$tmp[8]", $tmp[4], $tmp[5], $tmp[6]);
		$y++;
	}	
}
...
STEP: for my $i (0..$#in1) {
	...
	if ($chr1 ne $chr2 and $chr1 ne $chr0) { #CARs with one block only
		...
		#print "$chr1 $chr2 $scaf1 $scafStart1\n";
		...
		for my $j (0..$#in2) {	#for each scaf in ref2
			...
			if ($chr1 eq $chr2 and $scaf1 eq $scaf21 and $scafStart1 <= $scafEnd21 and $scafEnd1 >= $scafStart21 and (($minEnd - $minSt) >= (($scafEnd1-$scafStart1) * $perc)) ) {
				#print "HERE\t$scaf1 $scafStart1 $scaf21 $scafStart21\n";
