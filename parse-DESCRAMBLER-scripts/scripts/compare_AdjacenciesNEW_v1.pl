...
STEP: for my $i (0..$#in1) {
	...
	#my $data = $in1[$i];
	#my $data1 = $in1[$i+1];
	my ($chr0, $chrStart0, $chrEnd0, $scaf0, $scafStart0, $scafEnd0, $scafOr0) = @{$in1[$i-1]}; #previous line
	my ($chr1, $chrStart1, $chrEnd1, $scaf1, $scafStart1, $scafEnd1, $scafOr1) = @{$in1[$i]}; 	#current line
	my ($chr2, $chrStart2, $chrEnd2, $scaf2, $scafStart2, $scafEnd2, $scafOr2) = @{$in1[$i+1]};	#next line
	
	if ($chr1 ne $chr2 and $chr1 ne $chr0) { #CARs with one block only
		next STEP;	
		# for my $j (0..$#in2) {
		# 	my ($chr21, $chrStart21, $chrEnd21, $scaf21, $scafStart21, $scafEnd21, $scafOr21) = @{$in2[$j]};
		# 	my $minSt = max($scafStart1, $scafStart21);        
		# 	my $minEnd = min($scafEnd1, $scafEnd21);
		# 	if ($scaf1 eq $scaf21 and $scafStart1 <= $scafEnd21 and $scafEnd1 >= $scafStart21 and (($minEnd - $minSt) >= (($scafEnd1-$scafStart1) * $perc)) ) {
		# 		print OUT "$scaf1\t$scafStart1\t$scafEnd1\t$scafOr1\tna\tna\tna\tna\t$chr1\t$chr21\t";
		# 		print OUT "maintained\tused\talone\n";
		# 		$flag1 = "maintained\tfound";
		# 		next STEP;
		# 	}
		# 	else{ next STEP; }
		# 	# elsif ($scaf1 eq $scaf21 and $scafStart1 <= $scafEnd21 and $scafEnd1 >= $scafStart21 and (($minEnd - $minSt) < (($scafEnd1-$scafStart1) * $perc)) ) {
			# 	print OUT "$scaf1\t$scafStart1\t$scafEnd1\t$scafOr1\tna\tna\tna\tna\t$chr1\t$chr21\t";
			# 	print OUT "maintained\tused\tsmaller\n";
			# 	$flag1 = "maintained\tfound";
			# 	next STEP;
			# }
		#}
		...
		for my $j (0..$#in2) {	#for each scaf in ref2
			...
			if ($chr1 eq $chr2 and $scaf1 eq $scaf21 and $scafStart1 <= $scafEnd21 and $scafEnd1 >= $scafStart21 and (($minEnd - $minSt) >= (($scafEnd1-$scafStart1) * $perc)) ) {
				...
				if ($j == 0) { #If block in first line, only check next block
					my ($chr22, $chrStart22, $chrEnd22, $scaf22, $scafStart22, $scafEnd22, $scafOr22) = @{$in2[$j+1]};
				...
				if ($j == $#in2) { #If block in the last line, only check previous block
					...
					my ($chr20, $chrStart20, $chrEnd20, $scaf20, $scafStart20, $scafEnd20, $scafOr20) = @{$in2[$j-1]};
