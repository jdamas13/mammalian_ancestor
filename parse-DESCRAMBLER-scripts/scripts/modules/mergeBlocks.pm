...
sub mergeBlocks {
    ...
    START: for ( my $k = 0; $k <= scalar (@sorted_keys); $k += $add){ #for each line in file ordered by ref chr and ref start
        ...
        GO: for (my $i = 1; $i <= scalar (@sorted_keys) ; $i ++){
            ...
            if ($carOr1 !~ /-/ && $chr1 eq $chr2 && $carI1 eq $carI2 && $carOr1 eq $carOr2){ ## IF BOTH BLOCKS + ORIENTATION
                ...
                #print "$chr1 $cend $carS2 $gap\n";
                if ($gap > 0){ ## NOT SUCCESSIVE -> BREAK (start of 2nd block will be smaller than end of 1st block)
                    #print "+1 $carI2 $gap\n";
                    if ($carS2 < $cend && $carS2 > $cstart && $carE2 > $cstart){ #except if overlapping at beggining of second block, then merge
            ...
            if ($carOr1 =~ /-/ && $chr1 eq $chr2 && $carI1 eq $carI2 && $carOr1 eq $carOr2){ ## IF BOTH BLOCKS INVERTED 
