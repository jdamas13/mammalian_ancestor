...
LINE: for my $i (0..$#in1) {
	...
	if (${$in1[$i]}[1] ne ${$in1[$i+1]}[1] && ${$in1[$i]}[1] ne ${$in1[$i-1]}[1]) { #CARs with one block only
		#print "Skipping lone block RACF ".${$in1[$i]}[1]."\n";
		next LINE;	
	}
	elsif (${$in1[$i]}[1] ne ${$in1[$i+1]}[1] || $i == $#in1) {	#lastLine of CAR or lastLine of file
		#print "Skipping last line of ".${$in1[$i]}[1]." ".${$in1[$i]}[2]."\n";
		next LINE;
	}
		...
		if ($tarray[0] ne "") {
			...
			my $match2 = $t2array[0];
			if ($match_idx == $match2){
				#print ${$in1[$i]}[1]."\t".${$in1[$i]}[8]."\t".join("\t", @{$in1[$i]}[4..6])."\t";
				#print ${$in1[$i+1]}[8]."\t".join("\t", @{$in1[$i+1]}[4..6])."\t";
			...
			#print "$idx\t$firstFlag\t$chr1\t$chrStart1\n";
			my @tmp = &check_adjacency($i, $match_idx);
			if ($tmp[0] ne ""){
...
sub check_adjacency {
	my ($ori, $ori2, $block_flag, $adj_flag, $note_flag);
	my ($idx1, $idx2) = @_;

	if ($idx2 == 0) { #If block in first line, only check next block
		my @tmp = &check_blocks_fixed_idx($idx1, $idx2, "down");
		if ($tmp[0] ne ""){
			$block_flag = "2nd".$tmp[0];
			$adj_flag = $tmp[1];
			$note_flag = $tmp[2];
			return ($block_flag, $adj_flag, $note_flag); 
			exit;
	...
	if ($idx2 == $#in2) { #If block in the last line, only check previous block
		my @tmp = &check_blocks_fixed_idx($idx1, $idx2, "up");
		if ($tmp[0] ne ""){
			$block_flag = "2nd".$tmp[0];
			$adj_flag = $tmp[1];
			$note_flag = $tmp[2];
