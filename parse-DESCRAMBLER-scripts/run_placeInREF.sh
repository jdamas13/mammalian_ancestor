...
do
	...
	#Get ancestor RACFs in reference coordinates
	INFILE=$MAINDIR/$ANC/APCF_${REF}.merged.map.EH
	awk -F',' '{print $8","$9","$5","$6","$3","$4","$7","$1","$2","$2}' $INFILE >> $OUTFILE

	#Get ancestor CHRS in reference coordinates
	INFILE2=$MAINDIR/$SANC/APCF_${REF}.merged.map.EH.chrs.EH.merged
	awk -F',' '{print $8","$9","$5","$6","$3","$4","$7","$1","$2","$2}' $INFILE2 >> $OUTFILE
done

#Rename species and ancestors
perl $SCRIPTDIR/scripts/rename_and_sed.pl $OUTFILE ${REF}_DESC $SCRIPTDIR/species_rename.txt
#Create mysql file
